...
<body>
					...
					<div class="col-md-6 mx-auto">
						<div class="card rounded-0">
							 <div class="card-header mdb-color darken-4">
                            	<h3 class="text-center text-white mb-0"><span class="fa fa-sign-in"></span> Login Credentials for Users</h3>
                        	</div>
							<div class="card-body">
								<form class="form" method="post" role="form" autocomplete="off" id="formLogin">
									<div class="form-group">
										<label>Email</label>
										<input class="form-control form-control-lg rounded-0" type="email" name="email" id="admin_pass" required>
									</div>
									<div class="form-group">
									...
									<p>Don't Have an account?<a style="text-decoration: none;" href="user_register.php"> Sign Up Here</a></p>
	...
	<?php
		...
		if (isset($_POST['login'])) {
			...
			//query start
			$log_sql = "SELECT * FROM users WHERE Email = '$txtEmail' AND Password = '$txtPass'";
			$log_res = mysqli_query($conn, $log_sql);
			
			if (mysqli_num_rows($log_res) == 1) {
				$log_row = mysqli_fetch_assoc($log_res);
				//session variables
				$_SESSION['user_id'] = $log_row['UserID'];
